function resetAllInventory() {
	...
	dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean',
			'resetAllInventory', submitReturnCB);
};
...
$("#brandName_add").click(
		function() {
			...
			dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean',
					'addBrand', _brandName, submitCBBrand);
		});
...
function phoneReturn(id) {
	...
	dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean', 'returnPhone', id,
			submitReturnCB);
};

function phoneInventory(id) {
	dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean', 'inventoryPhone',
			id, function cb(_flag) {
				if (_flag) {
					alert('成功');
				} else {
					alert('失败');
...
function phoneDelete(id) {
	...
	jPrompt('请输入删除理由：', '删除操作', '', function(_reason) {
		...
		if (_reason != null) {
			_flag = dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean',
					'deletePhoneById', id, _reason, submitCB);
			if (_flag) {
				alert("删除成功");
			} else {
				alert("删除失败");
...
function doSell(id, sellPrice) {
	dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean', 'sellPhone', id,
			sellPrice, function(_flag) {
				if (_flag) {
					alert("卖出成功");
				} else {
					alert("卖出失败");
...
function doChangeSelledPrice(id, _price) {
	dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean', 'sellPriceChange',
			id, _price, function(_flag) {
				if (_flag) {
					alert("修改成功");
				} else {
					alert("修改失败");
...
function phoneChange(id, opt) {
	...
	if (_shopId != 0) {
		jConfirm('确定要转移到' + opt.options[_shopId].text + '吗?', '转移操作', function(
				_flag) {
			if (_flag) {
				dwr.engine._execute(_cfg_host + "/dwr/", 'WebPhoneBean',
						'changePhoneWithShop', id, _shopId, function(_flag) {
							if (_flag) {
								alert("转移成功");
								location.href = location.href;
							} else {
